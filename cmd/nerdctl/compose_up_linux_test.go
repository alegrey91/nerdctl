...
`, serviceRegular, testutil.NginxAlpineImage, serviceProfiled, testutil.BusyboxImage)
	fmt.Println(dockerComposeYAML)
	fmt.Println(testutil.Namespace)
	comp := testutil.NewComposeDir(t, dockerComposeYAML)
	defer comp.CleanUp()

	// here we run 'compose up --abort-on-container-exit' command
	base.ComposeCmd("-f", comp.YAMLFullPath(), "up", "--abort-on-container-exit").AssertOK()
	...
	expected := icmd.Expected{
		ExitCode: 1,
		Err:      `--abort-on-container-exit flag is incompatible with flag --detach`,
	}
	c.Assert(expected)
}
